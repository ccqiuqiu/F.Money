...
public class PtrViewHolder {
    ...
    PtrViewHolder(PtrLayout ptrLayout) {
        flag = FLAG_MOVE;
    }
    ...
    void offsetTopAndBottom(int dist) {
        ...
        if (canScroll()) {
            ...
            offsetY += dist;
            int maxHeight = ViewScrollChecker.dp2px(getView().getContext(),500);
//
            if (offsetY < 0) {
                offsetY = Math.max(offsetY, -maxHeight);
            }else{
                offsetY = Math.min(offsetY,maxHeight);
            }
        }
    }
