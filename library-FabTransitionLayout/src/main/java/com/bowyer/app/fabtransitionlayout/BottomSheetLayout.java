...
public class BottomSheetLayout extends FrameLayout implements View.OnTouchListener {
    ...
    public void expandFab(TextView tv) {
        mTv = tv;
        mFabType = FAB_EXPAND;
    ...
    public void contractFab() {
        if(mTv != null){
            InputMethodManager imm = ( InputMethodManager ) mTv.getContext( ).getSystemService( Context.INPUT_METHOD_SERVICE );
            if ( imm.isActive( ) ) {
                imm.hideSoftInputFromWindow( mTv.getApplicationWindowToken( ) , 0 );
            }
        }
        if (!isFabExpanded()) {
            return;
        ...
        fabSlideYAnim.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                if(mTv != null){
                    mTv.requestFocus();
                    InputMethodManager imm = ( InputMethodManager ) mTv.getContext( ).getSystemService( Context.INPUT_METHOD_SERVICE );
                    imm.showSoftInput(mTv,InputMethodManager.SHOW_FORCED);
                }
            }
        });
